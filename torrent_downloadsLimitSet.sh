...
function func_setDownloadUnlimited() { 
	if transmission-remote --torrent "${torrentID}" --info | grep -q "Upload Limit: 1 kB/s" ; then 
		transmission-remote --torrent "${torrentID}" --no-uplimit --bandwidth-normal 
	fi 
} 
...
function func_processList() { 
	for torrentID in "${A_torrentList[@]}" ; do 
		...
		elif transmission-remote --torrent "${torrentID}" --info | grep -q "Percent Done: 100%" ; then 
			func_setDownloadUnlimited 
		fi 
	done 
} 
